...
6. Update the GCP `env:` values in the GitHub Actions workflows if they differ from this
   project's defaults. CI authenticates with Workload Identity Federation, then loads secrets
   from Secret Manager. Optional GitHub secrets: `NEON_API_KEY` (fallback), `SENTRY_AUTH_TOKEN`,
   `CODECOV_TOKEN`.
